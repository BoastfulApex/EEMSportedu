...
@sync_to_async
def get_all_students_for_edu_admin(telegram_id: int) -> list:
    """Edu admin filialidagi barcha faol guruhlardagi tinglovchilar (guruh nomi bilan)."""
    from apps.students.models import Group
    from datetime import date
    today = date.today()
    admin = Administrator.objects.filter(telegram_id=int(telegram_id)).first()
    ...
    result = []
    for g in groups:
        for s in g.students.all().order_by('full_name'):
            result.append({'id': s.id, 'full_name': s.full_name, 'group_name': g.name})
    return result
